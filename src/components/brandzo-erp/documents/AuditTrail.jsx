  ...
  return (
    <ol className="space-y-2">
      {entries.map((e) => {
        ...
        return (
          <li key={e.id} className="flex items-start gap-3 bg-chip border border-line rounded-lg px-3 py-2">
            <span className="text-base leading-6">{action.emoji}</span>
            <div className="flex-1 min-w-0">
              <p className="text-sm text-ink-2">
                <strong className="text-ink">{e.byName}</strong>
                {role && (
              ...
              {e.note && <p className="text-xs text-muted mt-0.5 leading-relaxed">«{e.note}»</p>}
            </div>
