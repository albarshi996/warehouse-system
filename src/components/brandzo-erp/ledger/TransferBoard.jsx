              ...
              {pending.rows.map((r) => {
                ...
                return (
                  <tr key={r.id} className="border-t border-white/5">
                    <td className="py-2">
                      <a href={`${base}/dashboard/document?type=TRN&id=${r.id}`} className="font-bold text-brand-gold hover:underline">
                        {r.number || 'مسودّة'}
                      </a>
                      {r.driverName && <div className="text-gray-500">{r.driverName} · {r.vehiclePlate}</div>}
                    </td>
                    <td className="text-gray-300 whitespace-nowrap">{r.fromWarehouse} ← {r.toWarehouse}</td>
                    <td className="text-gray-400 whitespace-nowrap">{r.shipmentDate || '—'}</td>
                    <td className="text-gray-200">{num(r.shipped)}</td>
                    <td className="text-gray-200">{num(r.received)}</td>
            ...
            <tbody>
              {variances.rows.map((r, i) => (
                <tr key={i} className="border-t border-white/5">
                  <td className="py-2 text-gray-300 whitespace-nowrap">{r.transferNote}</td>
                  <td className="text-white">{r.description}</td>
                  <td className="text-gray-400 whitespace-nowrap">{r.fromWarehouse} ← {r.toWarehouse}</td>
                  <td className="text-gray-200">{num(r.shipped)}</td>
                  <td className="text-gray-200">{num(r.received)}</td>
                  <td className="font-bold" style={{ color: r.variance < 0 ? '#ef4444' : '#10b981' }}>
                    {r.variance > 0 ? `+${num(r.variance)}` : num(r.variance)}
                  </td>
                  <td className="text-gray-300">{r.reason || <span className="text-red-300">بلا سبب</span>}</td>
      ...
      {tab === 'transit' && (
        <div className="rounded-xl bg-black/20 border border-white/5 p-4">
          <p className="text-xs text-gray-400 mb-3 leading-relaxed">
            🚚 <b className="text-gray-200">مخزن النقل</b> موقعٌ وسيط يجب أن يعود <b className="text-emerald-300">صفرًا</b> بعد اكتمال كل استلام.
                ...
                <tbody>
                  {transitRows.map((b) => (
                    <tr key={b.id} className="border-t border-white/5">
                      <td className="py-2">
                        <div className="font-bold text-white">{b.sku || b.barcode}</div>
                        <div className="text-gray-400">{b.nameAr}</div>
                      </td>
                      <td className="text-gray-300">{b.batch || '—'}</td>
                      <td className="text-gray-400">{b.expiry || '—'}</td>
