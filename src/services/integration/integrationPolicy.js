 ...
 *
 * ⚠️ قُلبت 2026-08-13 (SAP-15 · تسدّ ف‑٣٧): كان الافتراض **دفعًا** لكلّ نوعٍ
 * عدا الأصناف، فكان غيابُ السياسة يعني الدفع — وهو عكس قرار المالك.
 * صار الافتراض **سحبًا للجميع**: من أراد دفعًا أعلنه صراحةً في لوحة التكامل،
 * ولا يُدفع شيءٌ لأنّ أحدًا نسي أن يقرّر.
 *
 * وهذا الخطّ الثاني لا الأوّل: حدُّ النقل مختومٌ في `odoo/directionGuard.js`،
 * فحتّى لو أُعيد نوعٌ إلى `push` هنا، لا تعبر كتابةٌ حتى يُفكّ الختم بقرار.
 */
export function defaultPolicyFor(_type) {
  // `_type` باقٍ في التوقيع عمدًا: الافتراض واحدٌ للجميع اليوم، لكنّ العقد يسمح
  // بتمييزٍ لاحق بلا تغيير كلّ نداء. وسابقةُ ذلك أنّ `items` كانت وحدها تُسحب.
  return {
    direction: 'pull',
    quantities: true,
    references: true,
