...
fix_coordinates.py
==================
يُصحّح إحداثيات DEFAULT_DATA في ملف retail-hub.astro بالاستعانة بـ:
  1. Nominatim API (إذا كان متاحاً في البيئة التي تشغّل منها السكريبت)
  2. قاعدة بيانات مضمّنة للإحداثيات المحقّقة لأحياء بنغازي كـ fallback

الاستخدام:
    python fix_coordinates.py              # مراجعة فقط (لا كتابة)
    python fix_coordinates.py --write      # تطبيق التعديلات وحفظ الملف
    python fix_coordinates.py --offline    # استخدام القاعدة المضمّنة فقط (بدون API)
"""
...
USER_AGENT  = "BenghaziRetailMap/1.0 (warehouse-doc; benghazi-coords-fix)"
SLEEP_SEC   = 1.1
# ─────────────────────────────────────────────────────────────────────────────
